...
class SettingsDocument {
    ...
    provideCompletionItems(position, _token) {
        ...
        if (location.path[0] === 'extensions.supportUntrustedWorkspaces' && location.path.length === 2 && location.isAtPropertyKey) {
            let alreadyConfigured = [];
            try {
                alreadyConfigured = Object.keys((0, jsonc_parser_1.parse)(this.document.getText())['extensions.supportUntrustedWorkspaces']);
            }
            catch (e) { /* ignore error */ }
            return (0, extensionsProposals_1.provideWorkspaceTrustExtensionProposals)(alreadyConfigured, range);
        }
